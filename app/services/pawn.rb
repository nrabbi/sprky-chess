class Pawn < ChessPiece
  ...
  def is_obstructed?(pieces, destination)
    ...
    destination.y > self.position.y ? (active_player = 1) : (active_player = 2)
    distance_check = (destination.y - self.position.y).abs

    if self.position.y == 1 && active_player == 1 || self.position.y == 6 && active_player == 2
